...
function initializeServiceWorker() {
  ...
  window.addEventListener('load', async () => {
    ...
    try {
      ...
      console.error("Could not register Service Workers!")
    }
...
async function getRecipes() {
  ...
  return new Promise(await function(resolve, reject) {
    ...
    //            declared above
    RECIPE_URLS.forEach(async function(recipe, index, array) {
      ...
      try {
        ...
        let fetchResponse = await fetch(recipe);
        ...
        //            "await" again
        let jsonRetrieval = await fetchResponse.json();

        // A8. TODO - Add the new recipe to the recipes array
        recipeArray.push(jsonRetrieval);
